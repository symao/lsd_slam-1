...
public:
    ...
    /** Sets ground truth depth (real, not inverse!) from a float array on level zero. Invalidates
     * higher levels. */
    void setDepthFromGroundTruth(const float* depth, float cov_scale = 1.0f);

    /** Prepares this frame for stereo comparisons with the other frame (computes some intermediate
     * values that will be needed) */
    void prepareForStereoWith(Frame* other, Sim3 thisToOther, const Eigen::Matrix3f& K,
                              const int level);
...
private:
    ...
    struct Data {
        ...
        // negative depthvalues are actually allowed, so setting this to -1 does NOT invalidate the
        ...
        // data from initial tracking, indicating which pixels in the reference frame ware good or
        // not. deleted as soon as frame is used for mapping.
    ...
    if (data.refPixelWasGood == 0) {
        ...
        if (data.refPixelWasGood == 0) {
            ...
            data.refPixelWasGood =
                (bool*)FrameMemory::getInstance().getBuffer(sizeof(bool) * width * height);
