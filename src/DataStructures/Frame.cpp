...
void Frame::takeReActivationData(DepthMapPixelHypothesis* depthMap) {
    ...
    if (data.validity_reAct == 0)
        data.validity_reAct =
            (unsigned char*)FrameMemory::getInstance().getBuffer(data.width[0] * data.height[0]);
    ...
    for (int y = 0; y < height0; y++) {
        for (int x = 0; x < width0; x++) {
            if (x > 0 && x < width0 - 1 && y > 0 &&
                ...
                pyrMaxGradient[x + y * width0] >= MIN_ABS_GRAD_CREATE && !isnanf(*depth) &&
                *depth > 0) {
...
void Frame::prepareForStereoWith(Frame* other, Sim3 thisToOther, const Eigen::Matrix3f& K,
                                 const int level) {
...
void Frame::buildImage(int level) {
    ...
    // if this is not the case, this still works except for the last * pixel, which will produce a
    // segfault. in that case, reduce this loop and calculate the last 0-3 dest pixels by hand....
    if (width % 8 == 0) {
...
void Frame::buildIDepthAndIDepthVar(int level) {
    ...
    if (level == 0) {
        printf(
            "Frame::buildIDepthAndIDepthVar(0): Loading depth from disk is not implemented yet! "
            "No-op.\n");
        return;
    }
...
void Frame::releaseIDepthVar(int level) {
    if (level == 0) {
        printf(
            "Frame::releaseIDepthVar(0): Storing depth variance on disk is not supported yet! "
            "No-op.\n");
        return;
    }
    FrameMemory::getInstance().returnBuffer(data.idepthVar[level]);
