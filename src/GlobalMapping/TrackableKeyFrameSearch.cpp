...
TrackableKeyFrameSearch::findEuclideanOverlapFrames(Frame* frame, float distanceTH, float angleTH,
...
Frame* TrackableKeyFrameSearch::findRePositionCandidate(Frame* frame, float maxScore) {
    ...
    for (unsigned int i = 0; i < potentialReferenceFrames.size(); i++) {
        ...
        msTrackPermaRef =
            0.9 * msTrackPermaRef + 0.1 * ((tv_end.tv_sec - tv_start.tv_sec) * 1000.0f +
        ...
        if (score < maxScore) {
            SE3 RefToFrame_tracked = tracker->trackFrameOnPermaref(
                potentialReferenceFrames[i].ref, frame, potentialReferenceFrames[i].refToFrame);
            Sophus::Vector3d dist =
                RefToFrame_tracked.translation() * potentialReferenceFrames[i].ref->meanIdepth;

            float newScore = getRefFrameScore(dist.dot(dist), tracker->pointUsage);
            float poseDiscrepancy =
                (potentialReferenceFrames[i].refToFrame * RefToFrame_tracked.inverse())
                    .log()
                    .norm();
            ...
            if (tracker->trackingWasGood && goodVal > relocalizationTH && newScore < bestScore &&
                poseDiscrepancy < 0.2) {
    ...
    if (bestFrame != 0) {
        if (enablePrintDebugInfo && printRelocalizationInfo)
            printf(
                "FindReferences for %d: Checked %d (%d). dist %.3f + usage %.3f = %.3f. pose "
                "discrepancy %.2f. TAKE %d!\n",
                (int)frame->id(), (int)potentialReferenceFrames.size(), checkedSecondary, bestDist,
                bestUsage, bestScore, bestPoseDiscrepancy, bestFrame->id());
        return bestFrame;
    } else {
        if (enablePrintDebugInfo && printRelocalizationInfo)
...
TrackableKeyFrameSearch::findCandidates(Frame* keyframe, Frame*& fabMapResult_out,
