...
Sim3 Sim3Tracker::trackFrameSim3(TrackingReference* reference, Frame* frame,
    ...
    for (int lvl = startLevel; lvl >= finalLevel; lvl--) {
        ...
        if (buf_warped_size <
                0.5 * MIN_GOODPERALL_PIXEL_ABSMIN * (width >> lvl) * (height >> lvl) ||
            buf_warped_size < 10) {
            diverged = true;
            return Sim3();
        }
    ...
    if (enablePrintDebugInfo && printTrackingIterationInfo) {
        ...
        printf("final res mean: %f meanD %f, meanP %f\n", finalResidual.mean, finalResidual.meanD,
               finalResidual.meanP);
    }

    // Make sure that there is a warp update at the final position to get the correct information
    // matrix
...
#if defined(ENABLE_SSE)
void Sim3Tracker::calcSim3BuffersSSE(const TrackingReference* reference, Frame* frame,
                                     const Sim3& referenceToFrame, int level, bool plotWeights) {
    calcSim3Buffers(reference, frame, referenceToFrame, level, plotWeights);
}
#endif

#if defined(ENABLE_NEON)
void Sim3Tracker::calcSim3BuffersNEON(const TrackingReference* reference, Frame* frame,
                                      const Sim3& referenceToFrame, int level, bool plotWeights) {
    calcSim3Buffers(reference, frame, referenceToFrame, level, plotWeights);
}
#endif

void Sim3Tracker::calcSim3Buffers(const TrackingReference* reference, Frame* frame,
                                  const Sim3& referenceToFrame, int level, bool plotWeights) {
    if (plotSim3TrackingIterationInfo) {
    ...
    for (; refPoint < refPoint_max; refPoint++, refGrad++, refColVar++) {
        ...
        *(buf_d + idx) = 1.0f / (*refPoint)[2];
        ...
        if (plotSim3TrackingIterationInfo) {
            ...
            setPixelInCvMat(&debugImageOldImageWarped, getGrayCvPixel((float)resInterp[2]), x, y,
                            (width / w));
            ...
            if (*(buf_warped_idepthVar + idx) >= 0) {
                setPixelInCvMat(&debugImageDepthResiduals,
                                getGrayCvPixel(128 + 800 * *(buf_residual_d + idx)), x, y,
                                (width / w));

                if (plotWeights) {
                    setPixelInCvMat(
                        &debugImageWeightD,
                        getGrayCvPixel(255 * (1 / 60.0f) * sqrtf(*(buf_weight_VarD + idx))), x, y,
                        (width / w));
                    setPixelInCvMat(
...
Sim3ResidualStruct Sim3Tracker::calcSim3WeightsAndResidualSSE(const Sim3& referenceToFrame) {
    ...
    for (int i = 0; i < buf_warped_size - 3; i += 4) {
        ...
        __m128 w_ds = _mm_rcp_ps(_mm_add_ps(
            ...
            _mm_mul_ps(g2s,
                       _mm_mul_ps(g2s, _mm_mul_ps(depthVarFacs, _mm_load_ps(buf_idepthVar + i))))));
        ...
        __m128 depthValid = _mm_cmplt_ps(
            zeros, _mm_load_ps(buf_warped_idepthVar +
    ...
    sumRes.numTermsD =
        SSEE(numTermsD, 0) + SSEE(numTermsD, 1) + SSEE(numTermsD, 2) + SSEE(numTermsD, 3);
...
Sim3ResidualStruct Sim3Tracker::calcSim3WeightsAndResidual(const Sim3& referenceToFrame) {
    ...
    float sum_rd = 0, sum_rp = 0, sum_wrd = 0, sum_wrp = 0, sum_wp = 0, sum_wd = 0, sum_num_d = 0,
          sum_num_p = 0;
    ...
    for (int i = 0; i < buf_warped_size; i++) {
        ...
        float weighted_abs_res = sv > 0 ? weighted_rd + weighted_rp : weighted_rp;
        float wh =
            fabs(weighted_abs_res < settings.huber_d ? 1 : settings.huber_d / weighted_abs_res);
    ...
    if (plotSim3TrackingIterationInfo) {
        printf("rd %f, rp %f, wrd %f, wrp %f, wd %f, wp %f\n ", sum_rd / sum_num_d,
               sum_rp / sum_num_p, sum_wrd / sum_num_d, sum_wrp / sum_num_p, sum_wd / sum_num_d,
               sum_wp / sum_num_p);
    }
    return sumRes;
}
...
void Sim3Tracker::calcSim3LGSSSE(LGS7& ls7) {
    ...
    for (int i = 0; i < buf_warped_size - 3; i += 4) {
        ...
        if (i + 3 < buf_warped_size) {
            ...
            ls6.updateSSE(J61, J62, J63, J64, J65, J66, _mm_load_ps(buf_warped_residual + i),
