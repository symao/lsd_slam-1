...
Relocalizer::Relocalizer(int w, int h, Eigen::Matrix3f K) {
    for (int i = 0; i < RELOCALIZE_THREADS; i++) running[i] = false;
...
void Relocalizer::updateCurrentFrame(std::shared_ptr<Frame> currentFrame) {
    ...
    if (displayDepthMap)
        Util::displayImage(
            "DebugWindow DEPTH",
            cv::Mat(currentFrame->height(), currentFrame->width(), CV_32F, currentFrame->image()) *
                (1 / 255.0f),
            false);
...
void Relocalizer::start(
    std::vector<Frame*, Eigen::aligned_allocator<lsd_slam::Frame*> >& allKeyframesList) {
            ...
            if (todoGoodVal > relocalizationTH) {
                ...
                for (Frame* nkf : todo->neighbors) {
                    SE3 nkfToFrame_init = se3FromSim3((nkf->getScaledCamToWorld().inverse() *
                                              ...
                                              .inverse();
                    SE3 nkfToFrame =
                        tracker->trackFrameOnPermaref(nkf, myRelocFrame.get(), nkfToFrame_init);
                ...
                if (numGoodNeighbours > numBadNeighbours || numGoodNeighbours >= 5) {
                    if (enablePrintDebugInfo && printRelocalizationInfo)
                        printf(
                            "RELOCALIZED! frame %d on %d (bestNeighbour %d): good %2.1f%%, usage "
                    ...
                    if (enablePrintDebugInfo && printRelocalizationInfo)
                        printf(
                            "FAILED RELOCALIZE! frame %d on %d (bestNeighbour %d): good %2.1f%%, "
                            "usage %2.1f%%, GoodNeighbours %d / %d\n",
                            myRelocFrame->id(), todo->id(), bestKF->id(),
                            100 * bestNeightbourGoodVal, 100 * bestNeighbourUsage,
                            numGoodNeighbours, numGoodNeighbours + numBadNeighbours);
                }
