...
Undistorter* Undistorter::getUndistorterForFile(const char* configFilename) {
    ...
    float ic[10];
    if (std::sscanf(l1.c_str(), "%f %f %f %f %f %f %f %f", &ic[0], &ic[1], &ic[2], &ic[3], &ic[4],
                    &ic[5], &ic[6], &ic[7]) == 8) {
...
UndistorterPTAM::UndistorterPTAM(const char* configFileName) {
    ...
    // l1 & l2
    if (std::sscanf(l1.c_str(), "%f %f %f %f %f", &inputCalibration[0], &inputCalibration[1],
                    &inputCalibration[2], &inputCalibration[3], &inputCalibration[4]) == 5 &&
    ...
    if (valid) {
        ...
        } else if (outputCalibration[0] == -2)  // "full"
        {
            ...
            ocy =
                std::max(trans_tl_radius / tl_radius, trans_tr_radius / tr_radius) * ofy * cy / fy;

            ofx = fx * ((hor) / (trans_hor)) * ((float)out_width / (float)in_width);
            ocx =
                std::max(trans_bl_radius / bl_radius, trans_tl_radius / tl_radius) * ofx * cx / fx;
...
void UndistorterPTAM::undistort(const cv::Mat& image, cv::OutputArray result) const {
    ...
    if (image.rows != in_height || image.cols != in_width) {
        printf(
            "UndistorterPTAM: input image size differs from expected input size! Not "
            "undistorting.\n");
        result.getMatRef() = image;
        return;
    }
...
int UndistorterPTAM::getInputWidth() const { return in_width; }

int UndistorterPTAM::getInputHeight() const { return in_height; }
...
UndistorterOpenCV::UndistorterOpenCV(const char* configFileName) {
    ...
    // l1 & l2
    if (std::sscanf(l1.c_str(), "%f %f %f %f %f %f %f %f", &inputCalibration[0],
                    &inputCalibration[1], &inputCalibration[2], &inputCalibration[3],
                    &inputCalibration[4], &inputCalibration[5], &inputCalibration[6],
                    &inputCalibration[7]) == 8 &&
        std::sscanf(l2.c_str(), "%d %d", &in_width, &in_height) == 2) {
        printf("Input resolution: %d %d\n", in_width, in_height);
        printf("In: %f %f %f %f %f %f %f %f\n", inputCalibration[0], inputCalibration[1],
               inputCalibration[2], inputCalibration[3], inputCalibration[4], inputCalibration[5],
               inputCalibration[6], inputCalibration[7]);
    } else {
        printf("Failed to read camera calibration (invalid format?)\nCalibration file: %s\n",
               configFileName);
        valid = false;
    ...
    for (int i = 0; i < 4; ++i) distCoeffs.at<float>(i, 0) = inputCalibration[4 + i];
...
int UndistorterOpenCV::getInputWidth() const { return in_width; }

int UndistorterOpenCV::getInputHeight() const { return in_height; }
