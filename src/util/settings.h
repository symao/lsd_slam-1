...
#define VAL_SUM_MIN_FOR_CREATE \
    (30)  // minimal summed validity over 5x5 region to create a new hypothesis for non-blacklisted
          // pixel (hole-filling)
#define VAL_SUM_MIN_FOR_KEEP \
    (24)  // minimal summed validity over 5x5 region to keep hypothesis (regularization)
#define VAL_SUM_MIN_FOR_UNBLACKLIST \
    (100)  // if summed validity surpasses this, a pixel is un-blacklisted.

#define MIN_BLACKLIST \
    -1  // if blacklist is SMALLER than this, pixel gets ignored. blacklist starts with 0.
...
#define FAIL_VAR_INC_FAC \
    1.1f  // after a failed stereo observation, the variance is increased by this factor.
#define MAX_VAR \
    (0.5f * 0.5f)  // initial variance on creation - if variance becomes larter than this,
...
#define MAX_ERROR_STEREO \
    (1300.0f)  // maximal photometric error for stereo to be successful (sum over 5 squared
               // intensity differences)
#define MIN_DISTANCE_ERROR_STEREO \
    (1.5f)  // minimal multiplicative difference to second-best match to not be considered
            // ambiguous.
...
public:
    inline DenseDepthTrackerSettings() {
        ...
        if (PYRAMID_LEVELS > 6)
            printf(
                "WARNING: Sim3Tracker(): default settings are intended for a maximum of 6 levels!");
