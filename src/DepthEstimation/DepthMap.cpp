...
void DepthMap::reset() {
    for (DepthMapPixelHypothesis* pt = otherDepthMap + width * height - 1; pt >= otherDepthMap;
         pt--)
        pt->isValid = false;
    for (DepthMapPixelHypothesis* pt = currentDepthMap + width * height - 1; pt >= currentDepthMap;
         pt--)
        pt->isValid = false;
}
...
void DepthMap::observeDepth() {
    threadReducer.reduce(boost::bind(&DepthMap::observeDepthRow, this, _1, _2, _3), 3, height - 3,
                         10);
    ...
    if (enablePrintDebugInfo && printObserveStatistics) {
        printf("OBSERVE (%d): %d / %d created; %d / %d updated; %d skipped; %d init-blacklisted\n",
               ...
               runningStats.num_observe_create_attempted, runningStats.num_observe_updated,
               runningStats.num_observe_update_attempted, runningStats.num_observe_skip_alreadyGood,
               runningStats.num_observe_blacklisted);
    }

    if (enablePrintDebugInfo && printObservePurgeStatistics) {
        printf(
            "OBS-PRG (%d): Good: %d; inconsistent: %d; notfound: %d; oob: %d; failed: %d; addSkip: "
            "%d;\n",
            activeKeyFrame->id(), runningStats.num_observe_good,
            runningStats.num_observe_inconsistent, runningStats.num_observe_notfound,
            runningStats.num_observe_skip_oob, runningStats.num_observe_skip_fail,
            runningStats.num_observe_addSkip);
    }
}

bool DepthMap::makeAndCheckEPL(const int x, const int y, const Frame* const ref, float* pepx,
                               float* pepy, RunningStats* const stats) {
    int idx = x + y * width;
    ...
    eplGradSquared =
        eplGradSquared * eplGradSquared / eplLengthSquared;  // square and norm with epl-length
...
bool DepthMap::observeDepthCreate(const int& x, const int& y, const int& idx,
    ...
    float error =
        doLineStereo(new_u, new_v, epx, epy, 0.0f, 1.0f, 1.0f / MIN_DEPTH, refFrame,
...
bool DepthMap::observeDepthUpdate(const int& x, const int& y, const int& idx,
    ...
    if (!activeKeyFrameIsReactivated) {
        ...
        if ((int)target->nextStereoFrameMinID - referenceFrameByID_offset < 0)
            ...
            refFrame =
                referenceFrameByID[(int)target->nextStereoFrameMinID - referenceFrameByID_offset];
    } else
    ...
    float error =
        doLineStereo(x, y, epx, epy, min_idepth, target->idepth_smoothed, max_idepth, refFrame,
                     refFrame->image(0), result_idepth, result_var, result_eplLength, stats);
...
void DepthMap::propagateDepth(Frame* new_keyframe) {
    ...
    if (new_keyframe->getTrackingParent() != activeKeyFrame) {
        printf(
            "WARNING: propagating depth from frame %d to %d, which was tracked on a different "
        ...
        for (int x = 0; x < width; x++) {
            ...
            if (trackingWasGood != 0) {
                ...
                if (residual * residual /
                            (MAX_DIFF_CONSTANT + MAX_DIFF_GRAD_MULT * destAbsGrad * destAbsGrad) >
                        1.0f ||
                    destAbsGrad < MIN_ABS_GRAD_DECREASE) {
                    if (enablePrintDebugInfo)
                        runningStats.num_prop_removed_colorDiff++;
            ...
            if (!targetBest->isValid) {
                ...
                *targetBest =
                    DepthMapPixelHypothesis(new_idepth, new_var, source->validity_counter);
                ...
                *targetBest = DepthMapPixelHypothesis(
                    merged_new_idepth, 1.0f / (1.0f / targetBest->idepth_var + 1.0f / new_var),
                    merged_validity);
            }
        }
    ...
    if (enablePrintDebugInfo && printPropagationStatistics) {
        printf(
            "PROPAGATE: %d: %d drop (%d oob, %d color); %d created; %d merged; %d occluded. %d "
            "col-dec, %d grad-dec.\n",
            runningStats.num_prop_attempts,
...
void DepthMap::regularizeDepthMapFillHolesRow(int yMin, int yMax, RunningStats* stats) {
    ...
    for (int y = yMin; y < yMax; y++) {
        for (int x = 3; x < width - 2; x++) {
            ...
            int val =
                io[2 + 2 * width] - io[2 - 3 * width] - io[-3 + 2 * width] + io[-3 - 3 * width];
...
void DepthMap::regularizeDepthMapFillHoles() {
    ...
    threadReducer.reduce(boost::bind(&DepthMap::regularizeDepthMapFillHolesRow, this, _1, _2, _3),
                         3, height - 2, 10);
...
void DepthMap::buildRegIntegralBuffer() {
    threadReducer.reduce(boost::bind(&DepthMap::buildRegIntegralBufferRow1, this, _1, _2, _3), 0,
                         height);
...
void DepthMap::regularizeDepthMap(bool removeOcclusions, int validityTH) {
    ...
    if (removeOcclusions)
        threadReducer.reduce(
            boost::bind(&DepthMap::regularizeDepthMapRow<true>, this, validityTH, _1, _2, _3), 2,
            height - 2, 10);
    else
        threadReducer.reduce(
            boost::bind(&DepthMap::regularizeDepthMapRow<false>, this, validityTH, _1, _2, _3), 2,
            height - 2, 10);

    if (enablePrintDebugInfo && printRegularizeStatistics)
        printf(
            "REGULARIZE (%d): %d smeared; %d blacklisted /%d new); %d deleted; %d occluded; %d "
            "filled\n",
        ...
        for (int x = 1; x < width - 1; x++) {
            if (maxGradients[x + y * width] > MIN_ABS_GRAD_CREATE) {
                ...
                currentDepthMap[x + y * width] = DepthMapPixelHypothesis(
                    idepth, idepth, VAR_RANDOM_INIT_INITIAL, VAR_RANDOM_INIT_INITIAL, 20);
            } else {
...
void DepthMap::initializeFromGTDepth(Frame* new_frame) {
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            if (!isnanf(idepthValue) && idepthValue > 0) {
                currentDepthMap[x + y * width] = DepthMapPixelHypothesis(
                    idepthValue, idepthValue, VAR_GT_INIT_INITIAL, VAR_GT_INIT_INITIAL, 20);
            } else {
                currentDepthMap[x + y * width].isValid = false;
                currentDepthMap[x + y * width].blacklisted = 0;
            }
...
void DepthMap::updateKeyframe(std::deque<std::shared_ptr<Frame> > referenceFrames) {
    ...
    for (std::shared_ptr<Frame> frame : referenceFrames) {
        ...
        if (frame->getTrackingParent() != activeKeyFrame) {
            printf(
                "WARNING: updating frame %d with %d, which was tracked on a different frame "
                "(%d).\nWhile this should work, it is not recommended.",
        ...
        if (frame->pose->trackingParent->frameID == activeKeyFrame->id())
            ...
            refToKf =
                activeKeyFrame->getScaledCamToWorld().inverse() * frame->getScaledCamToWorld();
    ...
    if (plotStereoImages) {
        ...
        cv::Mat oldest_refImage(oldest_referenceFrame->height(), oldest_referenceFrame->width(),
                                CV_32F, const_cast<float*>(oldest_referenceFrame->image(0)));
        cv::Mat newest_refImage(newest_referenceFrame->height(), newest_referenceFrame->width(),
                                CV_32F, const_cast<float*>(newest_referenceFrame->image(0)));
    ...
    msObserve = 0.9 * msObserve + 0.1 * ((tv_end.tv_sec - tv_start.tv_sec) * 1000.0f +
    ...
    // if(rand()%10==0)
    {
        ...
        gettimeofday(&tv_end, NULL);
        msFillHoles = 0.9 * msFillHoles + 0.1 * ((tv_end.tv_sec - tv_start.tv_sec) * 1000.0f +
                                                 (tv_end.tv_usec - tv_start.tv_usec) / 1000.0f);
        nFillHoles++;
    }
    ...
    regularizeDepthMap(false, VAL_SUM_MIN_FOR_KEEP);
    gettimeofday(&tv_end, NULL);
    msRegularize = 0.9 * msRegularize + 0.1 * ((tv_end.tv_sec - tv_start.tv_sec) * 1000.0f +
                                               (tv_end.tv_usec - tv_start.tv_usec) / 1000.0f);
    nRegularize++;
    ...
    if (!activeKeyFrame->depthHasBeenUpdatedFlag) {
        ...
        activeKeyFrame->setDepth(currentDepthMap);
        gettimeofday(&tv_end, NULL);
        msSetDepth = 0.9 * msSetDepth + 0.1 * ((tv_end.tv_sec - tv_start.tv_sec) * 1000.0f +
                                               (tv_end.tv_usec - tv_start.tv_usec) / 1000.0f);
        nSetDepth++;
    }
    ...
    if (enablePrintDebugInfo && printLineStereoStatistics) {
        printf(
            "ST: calls %6d, comp %6d, int %7d; good %6d (%.0f%%), neg %6d (%.0f%%); interp %6d / "
            "%6d / %6d\n",
            runningStats.num_stereo_calls, runningStats.num_stereo_comparisons,
            runningStats.num_pixelInterpolations, runningStats.num_stereo_successfull,
    ...
    if (enablePrintDebugInfo && printLineStereoFails) {
        printf(
            "ST-ERR: oob %d (scale %d, inf %d, near %d); err %d (%d uncl; %d end; zro: %d btw, %d "
            "no, %d two; %d big)\n",
            ...
            runningStats.num_stereo_invalid_unclear_winner, runningStats.num_stereo_invalid_atEnd,
            runningStats.num_stereo_invalid_inexistantCrossing,
            runningStats.num_stereo_invalid_noCrossing, runningStats.num_stereo_invalid_twoCrossing,
            runningStats.num_stereo_invalid_bigErr);
    }
}
...
void DepthMap::createKeyFrame(Frame* new_keyframe) {
    ...
    gettimeofday(&tv_end, NULL);
    msPropagate = 0.9 * msPropagate + 0.1 * ((tv_end.tv_sec - tv_start.tv_sec) * 1000.0f +
                                             (tv_end.tv_usec - tv_start.tv_usec) / 1000.0f);
    nPropagate++;
...
void DepthMap::addTimingSample() {
    ...
    float sPassed =
        ((now.tv_sec - lastHzUpdate.tv_sec) + (now.tv_usec - lastHzUpdate.tv_usec) / 1000000.0f);
    if (sPassed > 1.0f) {
        ...
        if (enablePrintDebugInfo && printMappingTiming) {
            printf(
                "Upd %3.1fms (%.1fHz); Create %3.1fms (%.1fHz); Final %3.1fms (%.1fHz) // Obs "
                "%3.1fms (%.1fHz); Reg %3.1fms (%.1fHz); Prop %3.1fms (%.1fHz); Fill %3.1fms "
                "(%.1fHz); Set %3.1fms (%.1fHz)\n",
                msUpdate, nAvgUpdate, msCreate, nAvgCreate, msFinalize, nAvgFinalize, msObserve,
                nAvgObserve, msRegularize, nAvgRegularize, msPropagate, nAvgPropagate, msFillHoles,
...
inline float DepthMap::doLineStereo(const float u, const float v, const float epxn,
    ...
    if (firstX <= 0 || firstX >= width - 2 || firstY <= 0 || firstY >= height - 2 || lastX <= 0 ||
        lastX >= width - 2 || lastY <= 0 || lastY >= height - 2) {
        return -1;
    }
    ...
    // - eplLength, min_idepth, max_idepth: determines search-resolution, i.e. the result's
    // variance.
    ...
    float val_cp_m2 =
        getInterpolatedElement(referenceFrameImage, cpx - 2.0f * incx, cpy - 2.0f * incy, width);
     ...
     * sum(e_i*e_i),sum(e_{i-1}*e_{i-1}),sum(e_{i+1}*e_{i+1}) and sum(e_i * e_{i-1}) and sum(e_i *
     * e_{i+1}), where i is the respective winning index.
     */
    ...
    while (((incx < 0) == (cpx > pClose[0]) && (incy < 0) == (cpy > pClose[1])) ||
        ...
        val_cp_p2 =
            getInterpolatedElement(referenceFrameImage, cpx + 2 * incx, cpy + 2 * incy, width);

        // hacky but fast way to get error and differential error: switch buffer variables for last
        // loop.
        float ee = 0;
        if (loopCounter % 2 == 0) {
        ...
        // otherwise: the last might be the current winner, in which case i have to save these
        // values.
        else {
            if (bestWasLastLoop) {
    ...
    if (useSubpixelStereo) {
        ...
        if (interpPre) {
            ...
            best_match_y -= d * incy;
            best_match_err =
                best_match_err - 2 * d * gradPre_this - (gradPre_pre - gradPre_this) * d * d;
            if (enablePrintDebugInfo)
            ...
            best_match_y += d * incy;
            best_match_err =
                best_match_err + 2 * d * gradPost_this + (gradPost_post - gradPost_this) * d * d;
            if (enablePrintDebugInfo)
    ...
    // check if interpolated error is OK. use evil hack to allow more error if there is a lot of
    // gradient.
    ...
    float alpha;  // d(idnew_best_match) / d(disparity in pixel) == conputed inverse depth derived
                  // by the pixel-disparity.
    if (incx * incx > incy * incy) {
        float oldX = fxi * best_match_x + cxi;
        float nominator =
            (oldX * referenceFrame->otherToThis_t[2] - referenceFrame->otherToThis_t[0]);
        ...
        float nominator =
            (oldY * referenceFrame->otherToThis_t[2] - referenceFrame->otherToThis_t[1]);
    ...
    if (plotStereoImages) {
        if (rand() % 5 == 0) {
            // if(rand()%500 == 0)
            //	printf("geo: %f, photo: %f, alpha: %f\n", sqrt(geoDispError), sqrt(photoDispError),
            // alpha, sqrt(result_var));

            // int idDiff = (keyFrame->pyramidID - referenceFrame->id);
            // cv::Scalar color = cv::Scalar(0,0, 2*idDiff);// bw

            // cv::Scalar color = cv::Scalar(sqrt(result_var)*2000, 255-sqrt(result_var)*2000, 0);//
            // bw
            ...
            //
            //			float pixelDistFound = sqrtf((float)((pReal[0]/pReal[2] -
            // best_match_x)*(pReal[0]/pReal[2] - best_match_x)
            //					+ (pReal[1]/pReal[2] - best_match_y)*(pReal[1]/pReal[2] -
            // best_match_y)));
            //
