...
public:
    ...
    /** Flag that blacklists a point to never be used - set if stereo fails repeatedly on this
     * pixel. */
    int blacklisted;
    ...
    inline DepthMapPixelHypothesis(const float &my_idepth, const float &my_idepth_smoothed,
                                   const float &my_idepth_var, const float &my_idepth_var_smoothed,
    ...
    inline DepthMapPixelHypothesis(const float &my_idepth, const float &my_idepth_var,
                                   const int &my_validity_counter)
        : isValid(true),
          blacklisted(0),
          nextStereoFrameMinID(0),
