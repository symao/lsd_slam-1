...
public:
    ...
    std::vector<FramePoseStruct*, Eigen::aligned_allocator<lsd_slam::FramePoseStruct*> >
    getAllPoses();
...
private:
    ...
    // SET & READ EVERYWHERE
    std::shared_ptr<Frame> currentKeyFrame;  // changed (and, for VO, maybe deleted)  only by
                                             // Mapping thread within exclusive lock.
    std::shared_ptr<Frame>
        trackingReferenceFrameSharedPT;  // only used in odometry-mode, to keep a keyframe alive
                                         // until it is deleted. ONLY accessed whithin
    ...
    // mutex to lock frame pose consistency. within a shared lock of this, *->getScaledCamToWorld()
    // is GUARANTEED to give the same result each call, and to be compatible to each other. locked
    // exclusively during the pose-update by Mapping.
    ...
    /** Calculates a scale independent error norm for reciprocal tracking results a and b with
     * associated information matrices. */
    float tryTrackSim3(TrackingReference* A, TrackingReference* B, int lvlStart, int lvlEnd,
                       bool useSSE, Sim3& AtoB, Sim3& BtoA, KFConstraintStruct* e1 = 0,
